...
const layoutBranch = (
  ...
  const xDir = snappedX >= 0 ? 1 : -1;
  const yDir = snappedY >= 0 ? 1 : -1;
  ...
  children.forEach((child) => {
    ...
    const childX = snapPosition(currentX + xDir * HORIZONTAL_GAP);
...
export const createFlowLayout = (
  ...
  const rootXDir = snappedRootX >= 0 ? 1 : -1;
  const rootYDir = snappedRootY >= 0 ? 1 : -1;

  rootChildren.forEach((child) => {
    const childY = snapPosition(cursorY + rootYDir * VERTICAL_GAP);
    const childX = snapPosition(rootX + rootXDir * HORIZONTAL_GAP);
