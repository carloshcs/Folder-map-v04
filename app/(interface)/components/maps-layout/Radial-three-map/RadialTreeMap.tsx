  ...
  useEffect(() => {
    ...
    const highlightNodes = (hoveredId: string | null) => {
      ...
      nodeGroups.each(function (node) {
        ...
        const hasCardElement = !cardElement.empty();
        const cardBaseFill = hasCardElement ? cardElement.attr('data-base-fill') : undefined;
        const baseFill = rect.attr('data-base-fill') || cardBaseFill;
        if (!baseFill) {
          return;
        }

        const cardBaseBorder = hasCardElement ? cardElement.attr('data-base-border') : undefined;
        const cardDimmedFill = hasCardElement ? cardElement.attr('data-dimmed-fill') : undefined;
        const cardDimmedBorder = hasCardElement ? cardElement.attr('data-dimmed-border') : undefined;
        const dimmedFill = rect.attr('data-dimmed-fill') || cardDimmedFill || shiftColor(baseFill, DIMMED_FILL_LIGHTEN);
        const dimmedBorder = cardDimmedBorder ||
          (cardBaseBorder ? shiftColor(cardBaseBorder, DIMMED_FILL_LIGHTEN * 0.65) : undefined);
        ...
        if (!hoveredId || (nodeIdentifier && relatedIds.has(nodeIdentifier))) {
          ...
          if (!cardElement.empty()) {
            ...
            if (cardBaseBorder) {
              cardElement.style('border-color', cardBaseBorder);
            }
          }
          return;
        }
