...
import { HORIZONTAL_GAP, VERTICAL_GAP, DRAG_VERTICAL_GAP } from '../config';
...
export const useDragHandlers = (lookups: Lookups, setters: Setters) => {
      ...
      setFlowNodes(nodes => {
        ...
        let nextNodes = nodes.map(node => {
          ...
          if (inBranch) {
            ...
            const nextX = crossedAxisX ? snapPosition(-start.x + adjExtraAfterMirrorX) : snapPosition(start.x + adjDeltaX);
            const nextY = crossedAxisY ? snapPosition(-start.y + adjExtraAfterMirrorY) : snapPosition(start.y + adjDeltaY);
            return { ...node, position: { x: nextX, y: nextY } };
          }
          if (!crossedAxisX && !crossedAxisY) return node;
          const nextX = crossedAxisX ? snapPosition(-start.x + extraAfterMirrorX) : snapPosition(start.x + totalDeltaX);
          const nextY = crossedAxisY ? snapPosition(-start.y + extraAfterMirrorY) : snapPosition(start.y + totalDeltaY);
          return { ...node, position: { x: nextX, y: nextY } };
        });
            ...
            if (directParentId && grandParentId) {
              ...
              if (branchRoots.length > 1) {
                ...
                if (draggedParentIndex !== -1) {
                  ...
                  if (firstBelow) {
                    const neededTop = draggedBranch.maxY + DRAG_VERTICAL_GAP;
  ...
  const handleNodeDragStop = useCallback(
    (id: string, _position?: XYPosition | null) => {
      ...
      setFlowNodes(nodes => {
        ...
        const updatedNodes = nodes.map(nodeItem => {
          ...
          if (!branch) {
            return { ...nodeItem, position: { x: snapPosition(nodeItem.position.x), y: snapPosition(nodeItem.position.y) } };
          }
...

      {
        ...
        setManualPositions(prev => {
          ...
          familySet.forEach(nodeId => {
            ...
            if (updatedNode) next.set(nodeId, { ...updatedNode.position });
          });
          return next;
        });
      }
          ...
          branchMutationsSnapshot.forEach(branch => {
            branch.branchIds.forEach(nodeId => {
              ...
              if (updatedNode) {
                ...
                next.set(nodeId, { x: snapPosition(stored.x + branch.deltaX), y: snapPosition(stored.y + branch.deltaY) });
              }
            });
          });
          return next;
    ...
    [childrenLookup, getDescendantIds, parentLookup, setCustomOrder, setFlowNodes, setManualPositions],
  );
